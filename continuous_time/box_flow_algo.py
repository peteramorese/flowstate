...
from pdf import std_gaussian_integral_hyperrectangle, standard_multivariate_gaussian_pdf
...
def grid_box_flow_algo(target_region : Rectangle, sample_volume : float, vf : VelocityField, dt : float, timesteps : int, box_propagator, axes : list[plt.Axes] = None):
    if axes is not None and vf.dim != 2:
        raise ValueError("Cannot visualize algorithm with dim != 2")
