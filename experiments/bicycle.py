...
def Cx0(x):
    return (std_gaussian_cdf(x[0]), std_gaussian_cdf(x[1]), std_gaussian_cdf(x[2]))

def Qx0(yx):
    return (std_gaussian_quantile(yx[0]), std_gaussian_quantile(yx[1]), std_gaussian_quantile(yx[2]))
...
def g(w, x):
    def g0(x, w):
        return x[0] + velocity_scale * (w[0] + 1) * np.cos(x[2]) * delta_t

    def g1(x, w):
        return x[1] + velocity_scale * (w[0] + 1) * np.sin(x[2]) * delta_t

    def g2(x, w):
        return x[2] + velocity_scale * (w[0] + 1) / wheelbase * np.tan(w[1]) * delta_t

    return (g0(x, w), g1(x, w), g2(x, w))

def g_inv(w, xp):
    def g0_inv(w, xp):
        return xp[0] - velocity_scale * (w[0] + 1) * np.cos(xp[2]) * delta_t

    def g1_inv(w, xp):
        return xp[1] - velocity_scale * (w[0] + 1) * np.cos(xp[2]) * delta_t

    def g2_inv(w, xp):
        return xp[2] - velocity_scale * (w[0] + 1) / wheelbase * np.tan(w[1]) * delta_t
...
def J_G(w, x):
    ...
    # dg0/dw0
    J[2, 0] = velocity_scale * np.cos(x[2]) * delta_t
    ...
    # dg0/dx2
    J[2, 4] = -velocity_scale * (w[0] + 1) * np.sin(x[2]) * delta_t

    # dg1/dw0
    J[3, 0] = velocity_scale * np.sin(x[2]) * delta_t
    ...
    # dg1/dx2
    J[3, 4] = velocity_scale * (w[0] + 1) * np.cos(x[2]) * delta_t

    # dg2/dw0
    J[4, 0] = velocity_scale * w[0] / wheelbase * np.tan(w[1]) * delta_t

    # dg2/dw1
    J[4, 1] = velocity_scale * (w[0] + 1) / (wheelbase * np.cos(w[1])**2) * delta_t
...
def J_Phiinv(yw, yx):
    ...
    # dx2/dyx2
    J[4, 4] = std_gaussian_quantile_deriv(yx[2])
...
def main():
    ...
    region = spatial.Rectangle([1, -0.1, -1], [1.2, 0.1, 1])
